  ...
  render() {
    const resetMainPage = NavigationActions.reset({
      ...
      actions: [
        NavigationActions.navigate({
          routeName: 'Main',
          params: { justCreatedBitmarkAccount: true }
        })
      ]
    });

    let requestNotification = () => {
      NotificationService.doRequestNotificationPermissions()
        .then((result) => {
    ...
    return (
      <BitmarkComponent
        ...
        content={
          <View style={[notificationStyle.body]} testID="NotificationComponent.view">
        ...
        footer={
          <View style={notificationStyle.enableButtonArea}>
            <TouchableOpacity
              style={[notificationStyle.enableButton]}
              onPress={requestNotification}
              testID="NotificationComponent.enable"
            >
              <Text style={notificationStyle.enableButtonText}>ENABLE</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                notificationStyle.enableButton,
                {
                  backgroundColor: 'white',
              ...
              testID="NotificationComponent.disable"
            >
              <Text style={[notificationStyle.enableButtonText, { color: '#0060F2' }]}>
                LATER
              </Text>
            </TouchableOpacity>
          </View>
