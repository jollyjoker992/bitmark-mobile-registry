...
export class BitmarkComponent extends React.Component {
  constructor(props) {
    ...
    let headerHeight = !this.props.header
      ? 0
      : this.props.headerHeight ||
        ios.constant.headerSize.height - ios.constant.headerSize.paddingTop;
    let footerHeight = !this.props.footer
      ? 0
      : this.props.footerHeight ||
        ios.constant.bottomTabsHeight + ios.constant.blankFooter;
    let keyboardExternalHeight = this.props.keyboardExternal
      ? this.props.headerHeight || ios.constant.autoCompleteHeight
      : 0;
  ...
  onKeyboardDidShow(keyboardEvent) {
    ...
    let contentHeight =
      ...
      this.state.keyboardExternalHeight -
      keyboardHeight;
  ...
  onKeyboardDidHide() {
    ...
    let contentHeight =
      this.state.bodyHeight - this.state.headerHeight - this.state.footerHeight;
  ...
  statusBarChanged(statusbarData) {
    ...
    let bodyHeight = statusBarHeight
      ? currentSize.height - statusbarData.frame.height
      : currentSize.height - ios.constant.headerSize.paddingTop;
    let contentHeight =
      bodyHeight -
      this.state.headerHeight -
      Math.max(
        this.state.footerHeight,
        this.state.keyboardHeight ? this.state.keyboardExternalHeight : 0
      ) -
  ...
  async doScroll(keyboardHeight, keyboardY, contentHeight, statusBarHeightChange) {
    ...
    listAnimations.push(
      Animated.spring(this.state.keyboardExternalBottom, {
    ...
    if (keyboardHeight > 0) {
      listAnimations.push(
        Animated.spring(this.state.keyboardExternalOpacity, {
          toValue: 1,
          duration: 200
        })
      );
      ...
      if (statusBarHeightChange === 0 && this.focusedElement) {
        this.focusedElement.measureInWindow((x, y, width, height) => {
          if (
            ...
            y + height >
              keyboardY -
                Math.max(this.state.keyboardExternalHeight, this.state.footerHeight)
          ) {
            let focusedElementYPosition =
              oldScrollYPosition +
              y +
              height -
              keyboardY +
        ...
        this.scrollRef.scrollTo({
          x: 0,
          y: this.scrollYPosition + statusBarHeightChange,
          animated: true
        });
      }
    }
  ...
  refresh() {
    setTimeout(() => {
      StatusBarManager.getHeight((result) => {
        let statusBarHeight = ios.config.isIPhoneX
          ? 0
          : result.height - ios.constant.headerSize.paddingTop;
        let bodyHeight = statusBarHeight
          ? currentSize.height - result.height
          : currentSize.height - ios.constant.headerSize.paddingTop;
        let contentHeight =
          bodyHeight - this.state.headerHeight - this.state.footerHeight;
        this.setState({ bodyHeight, contentHeight, statusBarHeight });
        Animated.spring(this.state.contentHeightAnimation, {
          toValue: contentHeight + this.state.footerHeight,
          duration: 200
    ...
    return (
      <View style={mainStyle}>
        ...
        >
          ...
          >
            <View
              style={[
                ...
                this.props.contentContainerStylep
              ]}
            >
          ...
          {this.state.keyboardExternalHeight > 0 &&
            this.state.keyboardHeight > 0 && (
              <Animated.View
                style={[
                  styles.keyboardExternal,
                  {
