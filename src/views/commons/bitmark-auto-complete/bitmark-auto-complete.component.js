  ...
  render() {
    return (
      <View style={styles.selectionArea}>
        <OneTabButtonComponent style={styles.nextButton} onPress={this.props.goToNextInputField}>
          <Image style={styles.nextButtonImage} source={require('assets/imgs/arrow_down_enable.png')} />
        </OneTabButtonComponent>
        <OneTabButtonComponent style={styles.prevButton} onPress={this.props.goToPrevInputField}>
        ...
        {this.props.dataSource && !!this.state.inputtedText && <View style={[styles.selectionList]}>
          <FlatList
            ...
            renderItem={({ item }) => {
              return (<OneTabButtonComponent style={styles.selectionItem} onPress={() => this.selectText(item.word, item.key)}>
                <Text style={[styles.selectionItemText, { color: this.state.selectedIndex === item.key ? 'blue' : 'gray' }]}>{item.word}</Text>
              </OneTabButtonComponent>)
            }}
          />
        </View>}
        <OneTabButtonComponent style={styles.doneButton} onPress={this.onDone} disabled={this.state.status !== BitmarkAutoCompleteComponent.statuses.done}>
