...
export class BitmarkComponent extends React.Component {
  constructor(props) {
    ...
    let headerHeight = !this.props.header ? 0 : (this.props.headerHeight || (constant.headerSize.height - constant.headerSize.paddingTop));
    let footerHeight = !this.props.footer ? 0 : (this.props.footerHeight || constant.bottomTabsHeight + constant.blankFooter);
    let keyboardExternalHeight = this.props.keyboardExternal ? (this.props.headerHeight || constant.autoCompleteHeight) : 0;
    let statusBarHeight = 0;
    let bodyHeight = currentSize.height - constant.headerSize.paddingTop;
  ...
  statusBarChanged(statusbarData) {
    let statusBarHeight = statusbarData.frame.height - constant.headerSize.paddingTop;
    let bodyHeight = statusBarHeight ? (currentSize.height - statusbarData.frame.height) : (currentSize.height - constant.headerSize.paddingTop);
  ...
  refresh() {
    setTimeout(() => {
      StatusBarManager.getHeight(result => {
        let statusBarHeight = config.isIPhoneX ? 0 : (result.height - constant.headerSize.paddingTop);
        let bodyHeight = statusBarHeight ? (currentSize.height - result.height) : (currentSize.height - constant.headerSize.paddingTop);
